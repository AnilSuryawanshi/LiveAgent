...
function getConfigurationItem(){
	...
	function getDislike(){	
	var feedback="dislike";
	 $.ajax({
		        type: 'GET',
				url: 'http://ec2-54-149-104-150.us-west-2.compute.amazonaws.com:8085/techM/feedback',
				data: {"feedback":feedback,
				"LdapuserName":Lusername		                
		               },
		        success: function(data){
					$(".feedback").html("");
...
function firstAjaxCall(Lusername, Lpassword,projectname,ipAdd,LdapEmailId) {
  //var success =function ldapuser(Lusername, Lpassword,projectname){
	 	 $.ajax({
		        type: 'POST',
				url: 'http://ec2-54-149-104-150.us-west-2.compute.amazonaws.com:8085/travelx/getajaxvalue',
...
function secondAjaxCall(Lusername, Lpassword,projectname,ipAdd,LdapEmailId,fullName) {
  //var success =function ldapuser(Lusername, Lpassword,projectname){
	 	 $.ajax({
		        type: 'POST',
				url: 'http://ec2-54-149-104-150.us-west-2.compute.amazonaws.com:8085/techM/getajaxvalue',
...
function thirdAjaxCall(Lusername, Lpassword,projectname,ipAdd,LdapEmailId) {
  //var success =function ldapuser(Lusername, Lpassword,projectname){
	 	 $.ajax({
		        type: 'POST',
				url: 'http://ec2-54-149-104-150.us-west-2.compute.amazonaws.com:8085/servicenow/getajaxvalue',
				data: {"Ldapuusername":Lusername,
		                "Ldappassword": Lpassword,
						"ipAdd":ipAdd,
						"LdapEmailId":LdapEmailId
...
function createForm(data){
	if(data.iCard.type!="taskList")
				...
				if(labelEntity==false)
					////change by icard show by icard end
				{	
				var lengthFormArray="";
		...
		$.post(locationInt, { 
			...
			//=======================================================================			
				...
				if(infoMessage.bothtechMTopFive.length>0)
				{
					var table = document.createElement("table");
					table.setAttribute("id", "detailslist", 0);
					var tableData="<table border='1' style='width:240px;'>";
				...
				if(infoMessage.bothIncidentdetailsTechM.length>0)
				{
					var table = document.createElement("table");
					table.setAttribute("id", "detailslist", 0);
					var tableData="<table border='1'style='width:240px;'>";
						for(var q=0;q<infoMessage.bothIncidentdetailsTechM.length;q++){
							tableData=tableData+"<tr><td><p><b>Request Number:</b><a id=" + infoMessage.bothIncidentdetailsTechM[q].incidentId + " onclick='markActiveLink(this)'>"+ " " + infoMessage.bothIncidentdetailsTechM[q].incidentId+"</a></p>"+
									   "<p><b>State: </b>"+infoMessage.bothIncidentdetailsTechM[q].state+"</p>"+
     								   "<p><b>Problem: </b>"+infoMessage.bothIncidentdetailsTechM[q].problems+"</p></td></td></tr>";									 														
					}
			...
			//=======================================================================
			 if(infoMessage.catalogValue!=undefined){
				if(infoMessage.catalogValue.length>0)
				{
					var tableData="<table border='1'style='width:240px;'>";					
					//alert("welcome nessa " + infoMessage.catalogValue);
					tableData=tableData+"1.Click on the relevant link.<br><a href='"+infoMessage.catalogValue+"' target='_blank'><i><b>Please click here</b></i><br></a>2.Login with your CDSID and password<br>3.Fill the required fields and submit the request You will receive a confirmation email regarding the request raised and you will receive periodic updates on your request status.";
									   ...
									   if(openend=="open_ended"||openended=="open_ended")
										{
											...
											$('.speech:last').append("<div  class='feedback'>Please give us feedback <img src='img/like-icon.png' id='like' onClick='getLike()'><img src='img/dislike-icon.png' id='dislike' onClick='getDislike()'></div>");
...
console.log("PORT "+portNumber);
    ...
    /*------------ Local variable ------------------*/
    var debug = true;
    var location = $('#ip').val() + ":8080/nessa/engine/dialog";
    ...
    //$("#login").click(function (e) {
        ...
        //code for hide ? start	here by vaibhav
		 ...
		 var userpass = s.split("=");
		 var username1 = userpass[1].split("&");
		 Lusername = username1[0];
		 var temppass = userpass[2].split("&");
		 var Lpassword = temppass[0];
		 var projectname = userpass[3];
		 var ipAddress = userpass[4].split("&");
		 var ipAdd = ipAddress[0];
		 var emailId = userpass[5].split("&");
		 var email = emailId[0];
		 var fullNameParam = userpass[6].split("&");
		 var fullName = fullNameParam[0];
		 
		//ldapuser(Lusername,Lpassword,projectname);
		firstAjaxCall(Lusername,Lpassword,projectname,ipAdd,email);
        secondAjaxCall(Lusername,Lpassword,projectname,ipAdd,email,fullName);
		thirdAjaxCall(Lusername,Lpassword,projectname,ipAdd,email);
		...
		getAssignmentGroupPassword();
		getConfigurationItem();
		getAssignmentList();
		getAdGroupAccess();   //change by tushar
        ...
        ipAddress = "http://192.168.1.206:9009/nessa/bot";
        //alert(ipAddress);
        ...
        if (username != '' && password != '' && password == passwordValid) {
            ...
            $.post(location,{ user: ipAdd},function (data, textStatus, jqXHR) {
				currentTime();
				    systemUtteranceContent = '<li class="mar-btm systemUtterance"> <div class="media-body pad-hor"> <div class="speech"><p id="systemUtterance">' + msg + '</p><p></p> </div></div></li>';
			  ...
			  menuOptionsItem+="<option value='Select from the options' selected='selected' disabled>Select from the options</option>";
			  for(var i=0;i<labelArray.length;i++){
					//alert("labelArray:::"+labelArray[i]);
					
					if(labelArray[i]=="Raise a Ticket")
				    {
                       menuOptionsItem+="<option value='#askForRaiseTicket'>" + "Raise a Ticket" + "</option>";
					//}else if(labelArray[i]=="Get the status of a request"){
					//   menuOptionsItem+="<option value='"+labelArray[i]+"'>" + labelArray[i] + "</option>";	
					...
					//   menuOptionsItem+="<option value='"+labelArray[i]+"'>" + labelArray[i] + "</option>";	
					   ...
					   menuOptionsItem+="<option value='#askForGetStatusOfTicket'>" + "Get status of existing Ticket" + "</option>";	
				    }   

			   }
					
			   $(".speech:last").append('<select id="menuDropdown" class="menuDropdown form-control" onChange="getMenuItemSelected(this.value)" >'+menuOptionsItem+' </select> <br>');
			   menuOptionsItem="";
	...
	$.post(locationInt, { 
			...
			if(infoMessage!="")
			{
				...
				if(infoMessage.image!="")
				{
				  ...
				  content = content+ "<br>" +"<div id='imgDiv' class='imgClass'><img src='"+infoMessage.image+"'></div>";
				}
				...
				if(infoMessage.audio!="")
				{
					content = content+ "<br>" +"<audio controls> <source src='horse.ogg' type='audio/ogg'><source src='"+infoMessage.audio+"' type='audio/mpeg'> Your browser does not support the audio tag. </audio>";
				}	
                ...
                if(infoMessage.documents!="")
				{
					content = content+'<br>'+"<div><a href='"+infoMessage.documents+"' target='_blank' >Click Here To Download Documents</a></div>";
				}
				...
				if(infoMessage.incidentDetailsList.length>0)
				{
					...
					//by vaibhav
					content = content+tableData+data.result.reply;
					//content = content+document.body.appendChild(table);
					//$(".speech").last().append('table');
				}	
				}
	//==============================================================================================
		      //-------doe techM volvo incidentdetailstechMTopFive???????????
...
//======================================================				
		   ...
		   }else
		   {
			   msg1 = data.result.reply;
                ...
                /*var userUtteranceValue = userQuestion;
				var userUtteranceVal="";
				var flag = userUtteranceValue.includes(":")
				if(flag)
				{
					var userUtterVal = userUtteranceValue.split(":");
		             userUtteranceVal = userUtterVal[1];
					
				}else
				{
					userUtteranceVal = userUtteranceValue;
				}	*/
				...
				//$('.speech:last').append('<input type="submit" value="add"/>');
				
                systemUtteranceContentResponse = '<li class="mar-btm systemUtterance"> <div class="media-body pad-hor"> <div class="speech" id="speechId"> <p id="">' + content +''+ msg1+ '</p> </div></div></li>';
                userUtteranceContent = '<li class="mar-btm userUtterance"><div class="media-body pad-hor speech-right"><div class="speech"><p id="userUtteranceValue">' + userUtteranceVal + '</p></div></div></li>';
				...
				if(labelEntity==false)	// for icard display check end
				...
				{	
				 ...
				 formDataObject = document.getElementById(newFormId);
				 if(formDataObject!=undefined||formDataObject!=null){
				 if(formDataObject.elements.length>0){
				 
				 var formInputTag = $("#"+newFormId+ " :input[type=text]");
				 var formSelectTag = $("#"+newFormId+" select");	 
				 var FormElements = formDataObject.elements;
				// var formInputTag = document.getElementsByTagName("input");
				 //var formSelectTag = document.getElementsByTagName("select");
										
					for(var j=0;j<formSelectTag.length;j++)
					{
						if(labelType=="list"&&labels=="Requested For:")
						{   var temp;
							if(iCard[i].value!="")
							{
									var enteredValue = iCard[i].value;
									var enteredValueToLowerCase = enteredValue.toLowerCase();
									
									strOption="";
									serviceNowGroupUser.forEach(function (data) {
										if(data == enteredValueToLowerCase){
                                           strOption+="<option value='"+data+"' selected>" + data + "</option>";
										    temp = data;
										} else {
											strOption+="<option value='"+data+"'>" + data + "</option>";
										}
                                     });
									 var link    = $(".custom-select").find("a"); //change sheetal 
									 link.find("span").text(temp);
									 //$(".custom-select").find("a").val(temp);
									// alert(link.find("span").text());
									$(".nameClassbox:last").html(strOption);
							}	
							
						}if(labelType=="list"&&labels=="Request Type:")
						{
							if(iCard[i].value!="")
							{
									var enteredValue = iCard[i].value;
									...
									requestType.forEach(function (data) {
										if(data == enteredValueToLowerCase){
                                           strOption1+="<option value='"+data+"' selected>" + data + "</option>";
										} else {
											strOption1+="<option value='"+data+"'>" + data + "</option>";
										}
                                     });
									$(".requestTypeClassbox:last").html(strOption1);
							}	
							
						}if(labelType=="list"&&labels=="Choose Group:")
						{
							if(iCard[i].value!="")
							{
									var enteredValue = iCard[i].value;
									...
									serviceNowGroup.forEach(function (data) {
										if(data == enteredValueToLowerCase){
                                           strOption2+="<option value='"+data+"' selected>" + data + "</option>";
										} else {
											strOption2+="<option value='"+data+"'>" + data + "</option>";
										}
                                     });
									$(".servicenowGroupClassbox:last").html(strOption2);
							}	
							
						}if(labelType=="list"&&labels=="AD Group Name:")
						{
							if(iCard[i].value!="")
							{
									var enteredValue = iCard[i].value;
									...
									ADGroupList.forEach(function (data) {
										if(data == enteredValueToLowerCase){
                                           strOption3+="<option value='"+data+"' selected>" + data + "</option>";
										   disabledSharedDisk();
										} else {
											strOption3+="<option value='"+data+"'>" + data + "</option>";
										}
                                     });
									$(".AdGroupClassbox:last").html(strOption3);
							}	
							
						} if(labelType=="list"&&labels=="shared disk access:")
						{
							if(iCard[i].value!="")
							{
									var enteredValue = iCard[i].value;
									...
									sharedDiskAccess.forEach(function (data) {
										if(data == enteredValueToLowerCase){
                                           strOption4+="<option value='"+data+"' selected>" + data + "</option>";
										} else {
											strOption4+="<option value='"+data+"'>" + data + "</option>";
						
						}
                                     });
									$(".SharedDiskAccessClassBox:last").html(strOption4);
							}	
						...
						if(labelType=="list"&&labels=="AD Group Access:")
						{
							if(iCard[i].value!="")
							{
									var enteredValue = iCard[i].value;
									...
									adGroupAccess.forEach(function (data) {
										if(data == enteredValueToLowerCase){
                                           strOption10+="<option value='"+data+"' selected>" + data + "</option>";
										} else {
											strOption10+="<option value='"+data+"'>" + data + "</option>";
						
						}
                                     });
									$(".AdGroupAccessClassBox:last").html(strOption10);
							}	
						...
						if(labelType=="list"&&labels=="Shared Disk:")
						{
							if(iCard[i].value!="")
							{
									var enteredValue = iCard[i].value;
									...
									sharedDisk.forEach(function (data) {
										if(data == enteredValueToLowerCase){
                                           strOption5+="<option value='"+data+"' selected>" + data + "</option>";
										   disabledADUser();
										} else {
											strOption5+="<option value='"+data+"'>" + data + "</option>";
										}
                                     });
									$(".SharedDiskClassBox:last").html(strOption5);

							}	
							
						}if(labelType=="list"&&labels=="Assignment Group:")
						{
							if(iCard[i].value!="")
							{
									var enteredValue = iCard[i].value;
									...
									assignmentGroupPassword.forEach(function (data) {
										if(data == enteredValueToLowerCase){
                                           strOption6+="<option value='"+data+"' selected>" + data + "</option>";
										} else {
											strOption6+="<option value='"+data+"'>" + data + "</option>";
										}
                                     });
									$(".assignmentGroupPasswordClassbox:last").html(strOption6);
							}	
							
						}
						if(labelType=="list"&&labels=="Configuration Item:")
						{
							if(iCard[i].value!="")
							{
									var enteredValue = iCard[i].value;
									...
									configurationItem.forEach(function (data) {
										if(data == enteredValueToLowerCase){
                                           strOption8+="<option value='"+data+"' selected>" + data + "</option>";
										} else {
											strOption8+="<option value='"+data+"'>" + data + "</option>";
										}
                                     });
									$(".configurationItemClassbox:last").html(strOption9);
							}	
							
						}if(labelType=="list"&&labels=="Assignment Groups:")
						{
							if(iCard[i].value!="")
							{
									var enteredValue = iCard[i].value;
									...
									$(".assignmentListClassbox:last").html(strOption8);
							}	
					...
					/*for(var j=0;j<formInputTag.length;j++)
					{
						if(labelType=="text"&&labels=="Approval Motivation")
						{
							if(iCard[i].value!="")
							{
									var enteredValue = iCard[i].value;
									...
									$("#textBox1").text(enteredValueToLowerCase);

							}	
							
						}
						if(labelType=="text"&&labels=="motivation for request")
						{
							if(iCard[i].value!="")
							{
									var enteredValue = iCard[i].value;
					  ...
					  if(newFormId=="")
					  {
						counter=counter+1;
						newFormId=formId+counter;
						//alert("while creating for m in converse::"+newFormId);
						//alert("formDataObject::"+formDataObject);
						 
					  }
					  formDataObject = createForm(data);
		...
		//alert("formDataObject while seeting my form content::"+formDataObject);
		myFormContent = formDataObject;
		//newFormId="";
		//newFormId ="";
		
		var actionArray = data.action.entities;
		if(actionArray.length>0)
		{
			for(var i=0;i<actionArray.length;i++){
			
			var formDataObject = document.getElementById(newFormId);
				 if(formDataObject!=undefined||formDataObject!=null){
				 if(formDataObject.elements.length>0){
				 ...
				 var formInputTag = document.getElementsByTagName("input");
					...
					for(var j=0;j<formInputTag.length;j++)
					{
						if(actionArray[i].label=="Approval Motivation:")
						{
							
							$(".approvaltextBoxClass:last").val(actionArray[i].value);

						}	
						if(actionArray[i].label=="motivation for request:")
						{
							
							$(".motivationtextBoxClass:last").val(actionArray[i].value);
							//$("#textBox1").val('this is sample note');
							
						}
						if(actionArray[i].label=="Problem:")
						{
							
							$(".problemTextBoxClass:last").val(actionArray[i].value);
							//$("#textBox2").val('this is sample note');
		...
		{
			newFormId="";
			//document.getElementById(newFormId).reset(); 
	        //document.getElementById("myFormClick").remove();
		}
 ...
 $( function() {
   ...
   /* function readURL(input) {
    ...
    $('.list-group .thumbnail').on('click', function(){
        ...
        $(this).closest('a').addClass('active');m 
